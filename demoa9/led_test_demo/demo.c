...
long demo_ioctl(struct file *filp, unsigned int cmd, unsigned long args)
{
	...
	int ret;
	led_desc_t *led;
	led = kzalloc(sizeof(led_desc_t), GFP_KERNEL);
	printk("---->%s--->%d\n",__func__,__LINE__);
printk("cmd = %d\n", cmd);
	printk("cmd = %d %d %d\n", cmd, FS_LED_ON, FS_LED_OFF);


	ret = copy_from_user(led,(void __user * )args,sizeof(led_desc_t));
		if(ret > 0){
			printk("copy_from_user failed\n");
			return -1;
		}
   printk("led->led_num = %d", led->led_num);
