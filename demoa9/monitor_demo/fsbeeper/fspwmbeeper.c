...
{
	writel ((readl(timer_base +TCON ) &(~(0xff)))| (1 <<0) | (1 <<3),timer_base +TCON ); 
}
...
{
	int ret = 0;
	beeper = kzalloc(sizeof(beep_desc_t), GFP_KERNEL);
	if(beeper == NULL)
	{
		printk("kzalloc is failed\n");
		return -1;
	}
	//	printk("---->%s--->%d\n",__func__,__LINE__);
	ret = copy_from_user(beeper,(void *)args,sizeof(beep_desc_t));
	if(ret > 0){
		printk("copy_from_user failed\n");
	}
